...
for(i in 1:dim(dataset)[1]){
  aux <- rowSums(replicate(nsim,eselectsim(ss_arm=round(dataset$samplesize_e1[i]/2),
                                           ...
                                           criteria="ARE",SS_r=FALSE))< c(-z.alpha,1))/nsim 
...
for(i in 1:dim(dataset)[1]){
  aux <- rowSums(replicate(nsim,eselectsim(ss_arm=round(dataset$samplesize_e1[i]/2),
                                          ...
                                          p0_e2=dataset$p0_e2[i],OR2=dataset$OR2[i],
                                          p0_ce=dataset$p0_ce[i],p_init=dataset$p_init[i],
                                          criteria="SS",SS_r=FALSE))< c(-z.alpha,1))/nsim 
...
save.image("C:/Users/mbofi/Dropbox/C5/Scripts/GitKraken/CBE_selection/results/results_H0False_withoutss_new.RData") 
