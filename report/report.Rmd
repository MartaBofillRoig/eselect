...
For each combination of ($p_1^{(0)},p_2^{(0)},OR_1,OR_2$), we plot the empirical power for each design (Endpoint selection with blinded estimation, Endpoint selection with unblinded estimation, Composite Endpoint, and Relevant Endpoint) obtained according to the correlation $\rho$.  
...
stable <- data.frame(Corr=sub$corr,ARE=round(sub$ARE,2),Dec_b=round(100*sub$decision_ES,2),Dec_ub=round(100*sub$decision_ES_ub,2)) 
