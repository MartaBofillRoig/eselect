...
# load("C:/Users/Marta/Dropbox/C5/Scripts/GitKraken/CBE_selection/results/results_bias.RData")
...
Main modifications with respect to previous versions:
  ...
  - Correction on the correlation estimator in the blinded approach. 
...
We run an extra simulation to assess the correlation estimator in both blinded and unblinded approaches. Using the same scenarios we considered before, we simulate each scenario $nsim = 1000$ times. For each run, we estimate the correlation using the blinded and unblinded approach. We then compute the mean and variance of this estimators. We define the bias as the difference between the mean value and the true value of the correlation in that scenario. 
