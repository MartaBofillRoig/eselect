...
<!-- The sample size is assumed to be equal for both groups $n^{(0)} = n^{(1)} = 500$, and the  significance level is $\alpha=0.025$. -->
...
for(i in 1:max(dataset$scenario)){
  sub=subset(dataset,dataset$scenario==i)
  p[[i]] <-ggplot(sub, aes(x=corr, y=ARE, color=as.factor(decision)))  +
    geom_point(size=2) + labs(y = "ARE", x="Correlation", color="Decision") + coord_cartesian(ylim = c(-0, 5))
}
marrangeGrob(p,ncol=2,nrow=2)
```
