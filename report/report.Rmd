...
```{r}
# load("C:/Users/Marta/Dropbox/C5/Scripts/GitKraken/CBE_selection/results_H0.RData")
...
summary(subset(dataset,dataset$gain_power != dataset$decision)[,c(19,20,21,26,27)])
```

### Case (1c) 

The following figure illustrates the empirical power obtained using the composite endpoint (CE), the endpoint 1 (RE) or the selected endpoint (ES). 
```{r, warning=F}
power_data <- data.frame(Power=c(dataset$Test_Power_ES,dataset$Test_Power_CE,dataset$Test_Power_RE),
           Endpoint=c(rep("ES",length(dataset$Test_Power_ES)),rep("CE",length(dataset$Test_Power_CE)),rep("RE",length(dataset$Test_Power_RE)))
           )
...
The following boxplots show the significance level obtained using the composite endpoint (CE), the endpoint 1 (RE) or the selected endpoint (ES). 
```{r, warning=F}
...
Conclusions:
  ...
  - The powers using the selected endpoint are larger than using the composite endpoint and the relevant endpoint.
  - The significance level is similar in the three situations.

Open questions:

  - Which are the input parameters anticipated for the design of the trial? ($p_1^{(0)},p_2^{(0)},OR_1,OR_2,\rho$) or ($p_1^{(0)},p_*^{(0)},OR_1,OR_*$)?
