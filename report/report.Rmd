...
library(ggpubr)
library(tidyverse)
# load("C:/Users/mbofi/Dropbox/C5/Scripts/GitKraken/CBE_selection/results/results_unblinded.RData")
...
We calculate the sample size to have $0.80$ power to detect an effect of $OR_1$ on the endpoint 1 at significance level $\alpha = 0.05$. 
...
Using the estimated probabilities for each composite component in each group ($p_1^{(0)},p_2^{(0)},p_1^{(1)},p_2^{(1)}$) and using the estimated pooled probability of the composite endpoint ($p_*$), we get an estimator of the correlation.
...
We do the same for the treatment group, and then we obtain a estimated value for the correlation in the treatment group $\rho^{(1)}$.

We calculate the ARE by using the estimated probabilities $p_1^{(0)},p_2^{(0)}$ and the mean of the two estimates for the correlation.
